...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\medskip
			\onslide<6->{The TDS $(\mathbb{T}, \alpha)$ is minimal iff $a \notin \mathbb{Q}$.}
			
			\medskip
			\onslide<7->{We have for $[x], [y] \in \mathbb{S}$ and $n \in \mathbb{Z}$ that
			\begin{align*}
				d_\mathbb{T}(\alpha^n([x]), \alpha^n([y])) = d_\mathbb{T}([x], [y]),
			\end{align*}
		...
		\begin{column}{0.5\textwidth}
			\begin{figure}[h]
				\begin{tikzpicture}
					...
					\onslide<4->\filldraw[color=red] ([shift={(230:2)}]0, 0) circle (.1) node[anchor=east, scale=1.5] {$\alpha([x])$};
					\onslide<5->\filldraw[color=red] ([shift={(370:2)}]0, 0) circle (.1) node[anchor=west, scale=1.5] {$\alpha^2([x])$};
					...
					\onslide<8->\filldraw[color=blue] ([shift={(250:2)}]0, 0) circle (.1) node[anchor=north, scale=1.5] {$\alpha([y])$};
					\onslide<9->\filldraw[color=blue] ([shift={(390:2)}]0, 0) circle (.1) node[anchor=west, scale=1.5] {$\alpha^2([y])$};
